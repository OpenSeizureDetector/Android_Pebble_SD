...
public class OsdUncaughtExceptionHandler implements Thread.UncaughtExceptionHandler {
     ...
     * Must run on main UI thread to avoid lifecycle issues on modern Android versions.
     */
    public void sendErrorMail(final StringBuilder errorContent) {
        // Use main thread Handler to show dialog
        ...
        mainHandler.post(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    // If we can't show the dialog, just log and exit
                    Log.e("OsdUncaughtExceptionHandler", "Failed to show crash dialog", t);
                    System.exit(1);
                }
            }
        });

        // Keep the crashed thread alive briefly to allow dialog to show
        try {
            Thread.sleep(2000);
